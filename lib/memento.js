...
var TEXT_REG = /^(text\/\w*|application\/(ecmascript|json|javascript|xml|x-javascript|x-markdown))(;\s?charset=UTF-8)?$/i,
	PARTIAL_CONTENT_MATCH = true,
	CONTENT_LIKE_THRESH = 0.75,
	SIMULATE_CAUSALITY = true;

function Memento(_matching_options) {
	this.stack = [];
	this.mods = arguments;
	this.matching_options = _matching_options;
}
	...
	for(var i = 0, l = stack.length; i < l; i++) {
		...
		if(exactUrlMatch(_req, stack[i])) {
			...
			if(_req.data && stack[i].data) {
				if(PARTIAL_CONTENT_MATCH) contentScoring.push(i);
				continue;
			}
		}

		if(partialUrlMatch(_req, stack[i])) {
...
function selectBestByContent(_memento, _indexes, _req) {
	...
	return maxLike > CONTENT_LIKE_THRESH ? selectAndRotate(_memento, best) : null;
}

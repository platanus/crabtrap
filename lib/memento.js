...
function Resource(_resource, _mods) {
	this.resource = _resource;
	this.mods = _mods;
}

Resource.prototype = {
	serve: function(_resp) {

		log.info("Serving: " + this.resource.method + ' ' + this.resource.url);
		log.debug("HTTP " + this.resource.statusCode);
		log.debug(JSON.stringify(this.resource.headers));

		_resp.statusCode = this.resource.statusCode;

		forOwn(this.resource.headers, function(k, v) { _resp.setHeader(k, v); });

		if(this.resource.content) {
			var contentType = this.resource.headers['content-type'],
				content = this.resource.content;

			// TODO: move this out of here!
			if(this.resource.encoding == 'utf-8' && contentType && HTML_REG.test(contentType)) {
			...
			var buf = new Buffer(content, this.resource.encoding);
			_resp.end(buf);
		} else {
			_resp.end();
		}
